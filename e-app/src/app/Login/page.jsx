  ...
  return (
    <section className="mt-8 bg-blue-200 rounded-full">
      ...
      <form className="flex flex-col text-center gap-5 max-w-xs mx-auto p-3 " onSubmit={handleFormSubmit} >
       ...
       name="email" value={email} onChange={ev => setEmail(ev.target.value)} disabled={loggingInProgress}
         />
        <input className="px-4 py-2 rounded-xl bg-gray-100 disabled:bg-red-400" type="password" placeholder="password" 
      name="password"  value={password} onChange={ev => setPassword(ev.target.value)} disabled={loggingInProgress}
               />
